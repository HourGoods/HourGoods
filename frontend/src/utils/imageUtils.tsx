...
// deal 이미지
export function uploadDealImage(file: any, filename: any, nickname: string) {
  ...
  const upload = new AWS.S3.ManagedUpload({
    params: {
      ...
      Key: `image/deal/${today}${nickname}${fileType}`, // 업로드 위치와 업로드할 파일명, 유저 프로필은 경로를 image/member-profile/로 넣어주세요
...
// profile 이미지
export function uploadProfileImage(file: any, filename: any, nickname: string) {
  const region = "ap-northeast-2";
  const bucket = "a204-hourgoods-bucket";
